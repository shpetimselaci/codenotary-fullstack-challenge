  ...
  const items = new Array(10).fill(undefined).map(
    () =>
      ({
        ...
        account_number: faker.fakerEN_US.finance.accountNumber(9),
        type: Math.random() > 0.5 ? 'receiving' : 'sending',
        iban: faker.fakerBASE.finance.iban({ countryCode: 'XK' }),
        address: faker.fakerEN_US.location.streetAddress(),
        amount: Number(faker.fakerEN_US.finance.amount({ min: 10, max: 100000, dec: 0 })),
        created_at: faker.fakerEN_US.date.recent().toISOString(),
      }) as Transaction,
  );
